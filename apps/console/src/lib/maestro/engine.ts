...
const AXIS_OWNER: Record<string, string> = {
  THY: "Melody Scout", VOC: "Vanessa", STY: "Metro Craft", TIM: "Analog Confessor",
  PER: "Dave", POST: "Eldrik", LYR: "Sage",
};

const AXIS_REVIEWERS: Record<string, string[]> = {
  THY: ["Dave", "Alan", "Anva"], VOC: ["Sage", "Dave", "Eldrik"],
  STY: ["Analog Confessor", "Sibling Architect", "Anva"], TIM: ["Eldrik", "Metro Craft", "Alan"],
  PER: ["Alan", "Vanessa", "Eldrik"], POST: ["Analog Confessor", "Vanessa", "Alan"],
...
async function speak(projectId: string, speakerId: string, text: string, kind: DialogueKind, addresses: string[] = []) {
  ...
  await appendDbDialogue({
    id: `dlg-${projectId}-${sequence}`, projectId, sequence, kind, speakerId,
    speakerType: speakerId === "operator" ? "HUMAN" : speakerId === "controller" ? "SYSTEM" : "EMPLOYEE",
    text, targetAddresses: addresses, evidenceRefs: [], createdAt: new Date().toISOString(),
  });
}
...
async function fillAxis(projectId: string, snapshot: ProjectSnapshot, axisId: string, seed: string, emit: Emit) {
  ...
  const result = await jsonModel(
    `You are ${owner.name}, the Maestro employee who owns ${owner.domain}. Core function: ${owner.function}.
You are a bounded SME, not the controller. Technical UST is canonical. NULL is a reserved address: every assigned address must receive a defensible value or an explicit null with rationale. Never skip an address. Artist-supplied quoted lyrics are immutable. Song Excellence is concurrent pressure.
Return JSON only: {"speech":"...","fills":[{"address":"...","value":any|null,"rationale":"..."}],"sem":[{"criterionId":"S1","score":0-5,"finding":"..."}]}.`,
    `ARTIST INPUT:\n${seed}\n\nASSIGNED ADDRESSES:\n${JSON.stringify(targets.map((a) => ({ address:a.address, field:a.subkeyName })))}\n\nSEM:\n${JSON.stringify(SEM_CRITERIA)}\nHandle every assigned address exactly once.`,
  );
  ...
  for (const atom of targets) {
    ...
    atom.evidenceRefs = [`owner:${owner.name}`, `reason:${fill?.rationale || "explicit-null"}`];
    atom.revision += 1;
    snapshot.sequence += 1;
...
async function pressureTurn(snapshot: ProjectSnapshot, staffName: string, seed: string, emit: Emit) {
...
Review the shared Technical UST only from your lane. Contradiction is first-class. Raise only material issues you can defend. Do not seize another domain. Score SEM honestly. Return JSON only: {"speech":"...","challenges":[{"address":"AXIS.Kn.Sn","severity":"observe|warn|challenge|block","issue":"...","recommendation":"..."}],"scores":{"S1":0-5,"S2":0-5,"S3":0-5,"S4":0-5,"S5":0-5,"S6":0-5,"S7":0-5,"S8":0-5,"S9":0-5,"S10":0-5,"S11":0-5,"S12":0-5}}.`,
    `ARTIST INPUT:\n${seed}\n\nCURRENT TECHNICAL UST:\n${JSON.stringify(compactUst(snapshot))}\n\nSEM:\n${JSON.stringify(SEM_CRITERIA)}`,
  );
  emit({ type:"pressure", employee:staffName, status:"done", challenges:Array.isArray(result.challenges) ? result.challenges.length : 0 });
  return { employee:staffName, speech:String(result.speech || `${staffName} reviewed the room.`), challenges:Array.isArray(result.challenges) ? result.challenges : [], scores:result.scores || {} };
}
...
async function reviseAddresses(projectId:string, snapshot:ProjectSnapshot, axisId:string, challenges:any[], seed:string, emit:Emit) {
  const owner = employee(AXIS_OWNER[axisId]);
  const addresses = Array.from(new Set(challenges.map((c:any) => String(c.address))));
  if (!addresses.length) return;
  const atoms = snapshot.technicalUst.filter((a) => addresses.includes(a.address));
  const result = await jsonModel(
    `You are ${owner.name}, lawful owner for ${owner.domain}. Peer challenges are routed back to you. Defend, revise, or explicitly preserve each challenged address. Do not modify artist-supplied quoted lyric text. Return JSON only: {"speech":"...","revisions":[{"address":"...","value":any|null,"rationale":"..."}]}.`,
...
function aggregateScores(all:Record<string,number>[]) {
...
async function generateSurfaces(snapshot:ProjectSnapshot, seed:string) {
  ...
  const [creative,summary,persona] = await Promise.all([
    jsonModel(`Compile the locked Technical UST into the Creative UST Suno lyrics-prompt surface. Mandatory block order: [Theory] [Voices] [Style] [Timbre] [Performance] [Post-Production] then ▸ LYRICS BLOCK. Preserve exact supplied lyrics. Return JSON {"text":"..."}.`, common),
    jsonModel(`Compile the locked Technical UST into a concise Show Summary style/production surface, approximately 1000 characters. Return JSON {"text":"..."}.`, common),
    jsonModel(`Compile the locked Technical UST into A/R persona surfaces. Return JSON {"styleLine":"<=150 chars","profile":"approximately 2000 chars"}.`, common),
  ]);
...
export async function runMaestroEndToEnd(projectId:string, emit:Emit) {
  ...
  try {
    ...
    for (const axis of TECHNICAL_UST) await fillAxis(projectId,snapshot,axis.id,seed,emit);
    snapshot.phase = "DRAFT_FREEZE";
    await updateDbProject(projectId,{phase:snapshot.phase,snapshot});
    await speak(projectId,"controller","Draft freeze applied. Every Technical UST address has a lawful disposition; the pressure room is open.","SYSTEM_NOTE");
    emit({type:"stage",name:"draft-freeze",status:"done"});

    let finalScores:Record<string,number> = {};
    let blocking:any[] = [];
    const maxRounds = Math.max(1, Math.min(5, Number(process.env.MAESTRO_MAX_ROUNDS || 3)));
    for (let round=1; round<=maxRounds; round+=1) {
      snapshot.phase = "ROUND_ROBIN";
      emit({type:"stage",name:`round-robin:${round}`,status:"running"});
      const turns = await Promise.all(STAFF.map((row)=>pressureTurn(snapshot,row[0],seed,emit)));
      for (const turn of turns) await speak(projectId,turn.employee,turn.speech,turn.challenges.length ? "CHALLENGE" : "EMPLOYEE_UTTERANCE",turn.challenges.map((c:any)=>String(c.address)));
      finalScores = aggregateScores(turns.map((t)=>t.scores));
      ...
      for (const challenge of blocking) {
        ...
        if (AXIS_OWNER[axis]) byAxis.set(axis,[...(byAxis.get(axis)||[]),challenge]);
      }
    ...
    const redPen = {
      ...
      composite, releaseFloor:SEM_RELEASE_FLOOR,
    ...
    if (!redPen.pass) {
      ...
      emit({type:"done",runId,status:"BLOCKED",redPen,semComposite:composite});
    ...
    emit({type:"done",runId,status:"COMPLETE",lockHash,packageHash,surfaces,triad:surfaces,semComposite:composite});
